...
WINDOW_ID="$(tmux new-window -d -c ~/Documents/Miscellaneous/AUR/"$PKGNAME" -P -F '#{window_index}' "$HOME/Documents/scripts/update-pkgbuild.sh; gdbus call --session --dest=org.freedesktop.Notifications --object-path=/org/freedesktop/Notifications --method=org.freedesktop.Notifications.Notify 'makepkg' 0 'Finished' 'Package build finished' 'Job for $PKGNAME finished.' '[]' '{\"desktop-entry\": <\"org.kde.konsole\">}' 5000 >/dev/null; exec zsh")"
