...
WINDOW_ID="$(tmux new-window -d -c ~/Documents/Miscellaneous/AUR/"$PKGNAME" -P -F '#{window_index}' "$HOME/Documents/scripts/update-pkgbuild.sh; ID=\$(gdbus call --session --dest=org.freedesktop.Notifications --object-path=/org/freedesktop/Notifications --method=org.freedesktop.Notifications.Notify 'makepkg' 0 'distributor-logo-archlinux' 'Package build finished' 'Job for $PKGNAME finished.' '[]' '{\"desktop-entry\": <\"org.wezfurlong.wezterm\">}' 5000 | grep -Eo 'uint32\s[[:digit:]]+'| cut -d' ' -f2); zsh; gdbus call --session --dest=org.freedesktop.Notifications --object-path=/org/freedesktop/Notifications --method=org.freedesktop.Notifications.CloseNotification \$ID")"
